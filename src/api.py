"""
api.py  —  Movie Recommender REST API
...
class RatingRequest(BaseModel):
    # Keys are movie_id strings (JSON keys are always strings), values range from 1 to 5
    ...
    def check_ratings(cls, v):
        ...
        for mid, r in v.items():
            if not mid.isdigit():
                raise ValueError(f"movie_id must be an integer, got '{mid}'.")
            if not (1.0 <= r <= 5.0):
                raise ValueError(f"Rating for movie {mid} must be between 1 and 5.")
        return v
